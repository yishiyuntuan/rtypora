...
- `tauri.conf.json` 已配置 CSP（`script-src 'self' 'wasm-unsafe-eval'`、`style-src 'self' 'unsafe-inline'`、`img-src` 含 asset/data/http(s)、其余 default-src 'self' 收紧）：新增前端能力（外链脚本/字体/框架/网络请求）时同步评估该策略；`index.html` 的首帧防白闪脚本因此外置为 `public/early-theme.js`（内联脚本在 `script-src 'self'` 下不执行，勿改回内联）。CSP 仅随自定义协议（生产构建）下发，dev 模式（devUrl 直连 Vite）不生效。
- 遵循最小权限原则：`capabilities/default.json` 仅授予窗口基础操作权限，新增插件或 core 调用时按需添加，不要整体放开。
- `tauri-plugin-autostart` 已注册插件与 `autostart:default` 权限（桌面平台），供前端按用户明确的偏好开关调用；不要在用户不知情的情况下注册开机自启。
- 渲染面净化：`SectionView.vue` 对 `<section>` 原文做 DOM 净化（剔除脚本类元素/on* 事件属性/javascript: 协议/srcdoc）后才 `v-html` 上屏——v-html 插入的 script 标签虽不执行，但事件属性会执行，净化不可省略；Rust 侧 `html.rs has_dangerous_attrs` 做字符引用解码后判定危险协议（防 `javas&#99;ript:` 实体拆分绕过）。
- 输入上限：`read_image_data_url` 超过 64MB 拒绝（内存 DoS）；`save_pasted_image` 的 `sub_dir`/`extension` 做目录穿越净化；`latex.rs` 公式输入超 64KB 拒绝（ratex 病态输入保险）。
