...
- `src/files.rs`：文件操作命令（`open_markdown_file` 弹框选读、`read_markdown_file` 按路径读、`list_dir` 列目录（`sort: "asc"|"desc"` 参数，排序在 Rust 完成）、`read_image_data_url` 本地图片读为 data URL、`save_file`/`save_file_as`/`save_html_as` 保存与导出（`source_name` 推导建议文件名，.md/.markdown → .html）、`save_pasted_image` 粘贴图片落盘、`pick_folder` 选择文件夹、`create_markdown_file` 新建 untitled.md）；文件对话框仅经 Rust 端 `tauri-plugin-dialog` 的 `DialogExt` 使用，前端无插件权限需求。
...
- `src/mermaid.rs`：Mermaid 图表渲染，移植自 velotype 渲染管线（`mermaid-rs-renderer`，Rust 端 SVG）。命令 `render_mermaid(source)` 接受 ```mermaid 围栏原文或图表正文（围栏在 Rust 端剥离），返回 SVG 文本（类型校验 + 错误标记检测 + 进程内缓存；webview 按 CSS 定尺寸，故省略 velotype 的磁盘缓存与缩放副本）。
...
- **前后端职责划分**：Vue 只负责渲染与 DOM 结构提取；Markdown 的解析、序列化、文本修改、统计、快捷判定、文件/目录/排序/导出等逻辑全部由 Rust 命令完成。前端不得在 JS 里生成/解析 Markdown 语法（DOM 只提取为 BlockDto JSON 交给 `serialize_markdown`）。**允许的例外**（均为视图层职责）：contenteditable 编辑态的 DOM 操作与光标管理（列表 Enter 续项/Tab 缩进/退格合并等，提交时经 Rust 序列化落源）、DOM→DTO 提取、调用 Rust 命令前的首字符预过滤（仅性能闸门，判定仍在 Rust）、展示格式化（路径拆分、列表序号、主题过滤等纯显示计算）。
