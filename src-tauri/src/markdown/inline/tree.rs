...
fn parse_until(
    ...
    while index < tokens.len() {
        ...
        if !inside_code && tokens[index].ch == '\\' {
            if let Some(escaped_len) = escaped_sequence_token_len(tokens, index) {
        ...
        if !inside_code {
            ...
            if tokens[index].ch == '<' {
                if let Some(next_index) = parse_inline_html_container(
                    tokens,
                    index,
                    extra_style,
                    extra_html_style.clone(),
            ...
            if tokens[index].ch == '<' {
                if let Some(next_index) = parse_autolink(
                    tokens,
                    index,
                    extra_style,
                    extra_html_style.clone(),
...
fn inline_html_semantic_style(name: &str, style: InlineStyle) -> InlineStyle {
    match name {
        ...
        "del" | "s" | "strike" => style.with_strikethrough(),
